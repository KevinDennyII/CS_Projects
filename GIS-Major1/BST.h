...
private:
   ...
   T* const FindHelper(T& toFind, BinNodeT<T>* sRoot); //helper function for find
...
public:
   ...
   T* const Find(T& D);             // return pointer to matching data,
                                          //  or NULL if not found

   void mapSortedListToBST(int Lo, long unsigned Hi, T List[], BST<T>& Tree);
	...
	// node is a leaf
		...
		else if((sRoot->Left != NULL) && (sRoot->Right != NULL))
		{
			...
			{	
				...
				Swapper->Left = sRoot->Left;//resetting root left or left subtree
				Swapper->Right = sRoot->Right;//resetting root right or right subtree
				delete sRoot;//deleting root
				sRoot = Swapper;//reassigning root
				
			}
			else
			{
				BinNodeT<T>* Swapper = sRoot->Right;
				//DeleteHelper(Swapper->Element, sRoot);
				Swapper->Left = sRoot->Left;
				delete sRoot;
				sRoot = Swapper;
			}
		...
		else if(sRoot->Left != NULL)
		{
			if(sRoot == Root)
			{	
				BinNodeT<T>* temp = Root;
				Root = Root->Left; //switching parent node pointer of sRoot to point to sRoot's left
				delete temp; //deleting  node
				return true;
			}
			else
			{
				BinNodeT<T>* temp = sRoot;
...
// Calls:       BST<T>::Insert() 
template <typename T>
void BST<T>::mapSortedListToBST(int Lo, long unsigned Hi, T List[], BST<T>& Tree) { 
  
   if ( Lo > Hi ) return;     // list is empty, nothing to do 
 
   int Mid = (Lo + Hi)/2;     // find midpoint of list 
   Tree.Insert( List[Mid] );  // add middle value to tree 
 
   mapSortedListToBST(Lo, Mid - 1, List, Tree);  // process left half 
   mapSortedListToBST(Mid + 1, Hi, List, Tree);  // process right half 
} 
...
// Calls:     FindHelper()
template <typename T>
T* const BST<T>::FindHelper(T& toFind, BinNodeT<T>* sRoot) 
{
	if (sRoot == NULL) 
	{
		cout << "A File ID has not been found, doesnt not exist in the tree or your tree is empty." <<endl;
		return NULL;
	}
...
void BST<T>::DisplayHelper(BinNodeT<T>* sRoot, ostream& Out, unsigned int Level) const
{
	if (Root == NULL) 
	{
		Out << "A File ID has not been found, doesnt not exist in the tree or your tree is empty."<<endl;
		return;
	}
	if (sRoot == NULL)
		return;
